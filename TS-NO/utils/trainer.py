...
from utils.data_utils import get_data_loader_novel
...
from utils.get_weights import get_weights_norm, get_weights_attention, get_weights_distance
...
class Trainer():
    ...
    def build_and_run(self):
        ...
        self.train_data_novel_loader, self.train_novel_dataset, self.train_novel_sampler = get_data_loader_novel(self.params, self.params.train_novel_path, dist.is_initialized(), train=True, pack=self.params.pack_data)
        ...
        self.val_data_novel_loader, self.val_novel_dataset, self.valid_novel_sampler = get_data_loader_novel(self.params, self.params.val_novel_path, dist.is_initialized(), train=False, pack=self.params.pack_data)
        ...
        if self.params.model == 'fno':
            ...
            #self.model = models.fno.fno(self.params)
        else:
        ...
        # 加载教师模型
        self.teacher_model = models.fno.fno(self.params).to(self.device)
        
        if dist.is_initialized():
            self.teacher_model = DistributedDataParallel(self.teacher_model,
                                                device_ids=[self.local_rank],
                                                output_device=[self.local_rank])
        
        # 给教师网络加载权重
        logging.info("Loading IC weights for teacher %s"%self.params.weights)
        self.load_teacher_model(self.params.weights)
        ...
        #    self.params.checkpoint_path = 'model_save/ckpt.tar'
    ...
    def train(self):
        ...
        print('正在计算软标签')
        modified_targets = []
        for i, (inputs, targets) in enumerate(self.train_data_novel_loader):
            if not self.params.pack_data: # send to gpu if not already packed in the dataloader
                inputs, targets = inputs.to(self.device), targets.to(self.device)
            u_soft = self.teacher_model(inputs)

            #targets = u_soft
            targets.data.copy_(u_soft.detach())
            modified_targets.append(targets.clone())
        ...
        # 计算算术平均值
        print('计算中心点')
        sum_data = torch.zeros((6, 128, 128), dtype=torch.float32)
        total_count = 0
        for i, (inputs_l, _) in enumerate(self.train_data_loader):
            sum_data += inputs_l.sum(dim=0)
            total_count += inputs_l.size(0)
        data_center = sum_data / total_count
        data_center = data_center[:, 0, 0]
        data_center = data_center.unsqueeze(1).to(self.device)
        ...
        for epoch in range(self.startEpoch, self.params.max_epochs):
            ...
            # train
            tr_time = self.train_one_epoch(modified_targets, data_center)
            ...
            '''
            # You can enable this to update the pseudo-solution
            print('Now：', epoch)
            if (epoch + 1) % 5 == 0:
                ...
                for i, (inputs, targets) in enumerate(self.train_data_novel_loader):
                    ...
                    u_soft = self.model(inputs)
                    targets.data.copy_(u_soft.detach())
                    modified_targets.append(targets.clone())
                    
                    del inputs, u_soft
                    torch.cuda.empty_cache()
                print('Update Pseudo-solutions')
            '''
    ...
    def train_one_epoch(self, modified_targets, data_center):
        tr_time = 0
        self.model.train()
        ...
        # 要同时对两个数据加载器遍历（batch均为64，所以最后的batch=64+64=128
        for i, ((inputs, targets), (inputs_unlabel, targets_soft)) in enumerate(zip(self.train_data_loader, self.train_data_novel_loader)):
            self.iters += 1
            ...
            if not self.params.pack_data: # send to gpu if not already packed in the dataloader
                inputs, targets = inputs.to(self.device), targets.to(self.device)
            if not self.params.pack_data: # send to gpu if not already packed in the dataloader
                inputs_unlabel, targets_soft = inputs_unlabel.to(self.device), targets_soft.to(self.device)
                
            tr_start = time.time()

            self.model.zero_grad()
            # 监督部分的损失 paired dataset loss
            u = self.model(inputs)
            loss_data_supervise = self.loss_func.data(inputs, u, targets)
            
            # 半监督部分的损失 Unpaired dataset loss
            # 首先计算权重

            # 最简单的版本，基于距离范数的加权
            #The simplest version, based on the weighting of the distance norm
            #re_weights = get_weights_norm(inputs_unlabel)   # get weights
            
            # 更一般的版本，基于注意力的加权
            # 这是基于点积加权的方法
            #A more general version, weighted based on attention
            attention_weights = get_weights_attention(inputs_unlabel, data_center)
            
            #torch.save(attention_weights.cpu(), 'attention_weights.txt')




            
            u_pred = self.model(inputs_unlabel)
            #loss_data_soft = self.loss_func.data(inputs_unlabel, u_pred, modified_targets[i])  # 这是不加权的损失
            #mse_loss.sum(dim=(1, 2, 3)).sum()/u_pred.shape[0] 与上面等价

            # 下面开始加权
            mse_loss = torch.nn.functional.mse_loss(u_pred, modified_targets[i], reduction='none')
            mse_loss = mse_loss.sum(dim=(1, 2, 3)).unsqueeze(1)
            weighted_losses = mse_loss * attention_weights
            
        
            loss_data_soft = weighted_losses.sum()/u_pred.shape[0]
            
           
    
            # 将不同部分的损失相加
            loss_data = loss_data_supervise + loss_data_soft    
            #print('Minibatch:', i, 'Supervise Loss:', loss_data_supervise, 'semi-supervise:', loss_data_soft)
    ...
    def load_teacher_model(self, checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location='cuda:{}'.format(self.local_rank)) 
        try:
            self.teacher_model.load_state_dict(checkpoint['model_state'])
        except:
            new_state_dict = OrderedDict()
            for key, val in checkpoint['model_state'].items():
                name = key[7:]
