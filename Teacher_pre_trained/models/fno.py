...
class FNN2d(nn.Module):
    def __init__(self, modes1, modes2,
        ...
        # 对比分支
        #self.fc_emb11 = nn.Linear(layers[-1], fc_dim)
        #self.fc_emb12 = nn.Linear(fc_dim, out_dim)
        
        #self.fc_emb21 = nn.Linear(128*128, 2048)
        #self.fc_emb22 = nn.Linear(2048, 128)
    ...
    def forward(self, x):
        ...
        for i, (speconv, w) in enumerate(zip(self.sp_convs, self.ws)):
            ...
            if i != length - 1:
                x = self.activation(x)
            x = self.dropout(x)
        # 现在这里x是 batch*64*128*128
        
        # 这一部分是对比分支，将傅里叶层的输出转换为低维embedding
        #x_emb = x.clone()
        #x_emb = x_emb.permute(0, 2, 3, 1)
        #x_emb = self.fc_emb11(x_emb)
        #x_emb = self.activation(x_emb)
        #x_emb = self.dropout(x_emb)
        #x_emb = self.fc_emb12(x_emb)
        #x_emb = self.dropout(x_emb)
        #x_emb = x_emb.permute(0, 3, 1, 2)    # 现在是batch_size * 1*128*128
        
        #x_emb = x_emb.reshape(x_emb.shape[0], -1)  # 现在是batch_size * (128*128)
        
        #x_emb = self.fc_emb21(x_emb)
        #x_emb = self.activation(x_emb)
        #x_emb = self.dropout(x_emb)
        #x_emb = self.fc_emb22(x_emb)   # 现在的大小是batch_size * 128， 用于对比学习
