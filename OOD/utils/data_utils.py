...
def get_data_loader(params, location, distributed, train=True, pack=False):
    ...
    if not pack:
        ...
        dataloader = torch.utils.data.DataLoader(tensor_dataset, batch_size=int(batch_size), shuffle=False)
    return dataloader, dataset, sampler

def get_data_loader_novel(params, location, distributed, train=True, pack=False):
    transform = torch.from_numpy
    dataset = PDESolns_novel(params, location, transform, train)
    sampler = DistributedSampler(dataset, shuffle=train) if distributed else None
    if train:
        batch_size = params.batch_size_novel
    else:
        batch_size = params.valid_batch_size_novel
    if not pack:
        dataloader = DataLoader(dataset,
                                batch_size=int(batch_size),
                                num_workers=params.num_data_workers,
...
class PDESolns_novel(Dataset):
    ...
    def _get_files_stats(self):
        ...
        self.n_samples /= self.subsample
        self.n_samples = int(self.n_samples)
        logging.info("Found novel_data at path {}. Number of examples: {}. Image Shape: {} x {}".format(self.location, self.n_samples, self.img_shape_x, self.img_shape_y))
        if hasattr(self.params, "scales_path"):
            self.novel_scales = np.load(self.params.scales_novel_path)
            self.novel_scales = np.array([s if s != 0 else 1 for s in self.novel_scales]) 
            self.novel_scales = self.novel_scales.astype('float32')
            measure_x = self.novel_scales[-2] / self.img_shape_x
            measure_y = self.novel_scales[-1] / self.img_shape_y
            self.measure = measure_x * measure_y
            logging.info("novel_Scales for PDE are (source, tensor, sol, domain): {}".format(self.novel_scales))
            logging.info("Measure of the set is lx/nx * ly/ny =  {}/{} * {}/{}".format(self.novel_scales[-2], self.img_shape_x, self.novel_scales[-1], self.img_shape_y))
    ...
    def __getitem__(self, idx):
        ...
        if self.novel_scales is not None:
            f_norm = np.linalg.norm(X[0]) * self.measure
            f_scaling = f_norm / self.novel_scales[0]
            X = X / f_scaling # ensures that 10f and 10k for example, have the same input

            # scale the tensors
            X[self.in_channels:] = X[self.in_channels:] / self.novel_scales[self.in_channels:(self.in_channels + self.tensor_shape), None, None]

        X = self.transform(X)
        # 这是一个方程的6个输出，1个源项的值，5个系数值
        y = self.transform(self.data[local_idx,self.in_channels:])
        # 这是u值
